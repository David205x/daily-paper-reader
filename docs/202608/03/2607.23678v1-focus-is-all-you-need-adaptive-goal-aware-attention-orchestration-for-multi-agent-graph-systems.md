...
tldr: 基于图结构的多智能体系统面临注意力分配难题，现有方法常均匀执行图组件，浪费资源。本文提出自适应目标感知注意力编排框架AGAO，将Transformer式注意力扩展到工作流级协调，通过目标感知、拓扑感知和资源感知三种注意力动态评估智能体重要性，将静态图变为聚焦关键推理路径的自适应系统。实验表明AGAO在提升任务效果的同时降低计算开销、延迟和令牌消耗，确立注意力工程方向。
source: arxiv
...
motivation: 图式多智能体系统均匀执行组件导致资源浪费，缺乏根据用户目标动态分配注意力的机制。
method: 提出AGAO框架，结合目标感知、拓扑感知和资源感知三种注意力动态评估智能体重要性与预算分配。
result: 在多种多智能体任务上，AGAO相比现有图执行策略提升任务效果并降低计算、延迟和令牌消耗。
conclusion: 注意力编排为可扩展智能多智能体系统提供了新范式，推动注意力工程研究方向。
---

## 摘要
大型语言模型（LLM）使得自主智能体能够进行推理、规划和使用工具。近年来，系统越来越多地将这些智能体组织为专门化、相互连接的节点图。尽管基于图的编排支持灵活的分解与协调，但它也带来了一个关键挑战：\textbf{注意力分配}。随着工作流的增长，现有方法通常统一执行图组件，在无关或低影响任务上浪费资源。我们引入了\textbf{注意力编排}（Attention Orchestration），这是一种将Transformer风格的注意力从标记表示扩展到工作流级智能体协调的范式。我们的框架\textbf{自适应目标感知注意力编排}（AGAO）基于用户目标、图依赖关系和计算约束动态估计智能体的重要性。AGAO结合了三个组件：（1）目标感知注意力，衡量用户目标与智能体能力之间的语义相关性；（2）拓扑感知注意力，建模智能体图中的结构依赖关系；（3）资源感知注意力，在异构智能体之间分配预算和执行优先级。这些机制共同将静态智能体图转化为自适应系统，将计算集中在目标关键的推理路径上。在多种多智能体工作负载上的实验表明，与现有的基于图的执行策略相比，AGAO提高了任务有效性，同时减少了不必要的计算、延迟和令牌消耗。我们的工作确立了\textbf{注意力工程}（Attention Engineering）作为可扩展、智能多智能体系统的一个方向。代码：https://github.com/MingzhouFan97/AGAO。
