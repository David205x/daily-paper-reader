...
tldr: 个性化联邦强化学习现有方法过度依赖外部奖励信号，在稀疏或非平稳环境中缺乏有效探索。本文提出EDPFRL-IM框架，通过客户端内在好奇心驱动探索，并利用随机网络蒸馏产生内在奖励以促进新状态空间的发现；服务器仅发送全局探索先验并收集最小新颖性摘要，兼顾协调探索与隐私保护。在基准测试中，该方法在延迟和稀疏奖励任务上显著提升了策略个性化和样本效率。
source: arxiv
...
motivation: 现有PFRL方法依赖奖励信号，忽视探索，导致在稀疏奖励或非平稳环境下策略学习效率低下。
method: 客户端用RND内在奖励补充外在奖励，服务器共享探索先验并聚合最小新颖性摘要，实现隐私保护的协同探索。
result: 在延迟和稀疏奖励基准上，策略个性化和样本效率均优于平均PFRL方法。
conclusion: EDPFRL-IM在保护隐私的同时，为联邦强化学习引入灵活的探索机制，提升复杂环境适应性。
---

## 摘要
个性化联邦强化学习（PFRL）采用去中心化的方式来存储和访问基于过往经验的信息，同时在学习每个客户端策略时保持各客户端数据的隐私。当前许多PFRL方法严重依赖现有的强化学习奖励信号来为每个客户端推导最优策略，从而忽视了在非平稳或稀疏奖励环境中的探索。在本工作中，我们提出了一种新的探索驱动框架——基于内在动机的探索驱动个性化联邦强化学习（EDPFRL-IM），该框架利用每个客户端固有的好奇心驱动探索来促进局部探索并保护客户端隐私。此外，为了通过探索在先前未探索的状态空间中促进策略发现，客户端将内在随机网络蒸馏（RND）信号添加到其外在奖励中。另外，服务器无法访问客户端的原始经验或局部梯度估计；相反，服务器发送全局探索先验信息，并从每个客户端收集最小的新颖性摘要，以实现客户端之间多样且协调的探索。基准环境中的实验表明，我们的框架在策略个性化和样本效率方面优于平均PFRL基准，尤其是在延迟和稀疏奖励系统中。总体而言，EDPFRL-IM能够在保护客户端隐私的同时，将灵活的探索学习结构集成到联邦强化学习系统中。
