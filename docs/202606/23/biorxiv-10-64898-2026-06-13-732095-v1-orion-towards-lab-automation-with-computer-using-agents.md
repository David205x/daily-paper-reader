...
tags: ["query:rl"]
score: 8.0
evidence: 用于实验室任务自动化的大语言模型智能体
tldr: "实验室计算工作流受限于专业软件手动操作和知识整合瓶颈。Orion结合大语言模型与终端执行、GUI控制和自适应多步推理，自动操作科学软件、挖掘网络资源并执行端到端分析。在基准测试中，生物数据库和文献检索准确率超90%；学会使用CellProfiler和QuPath进行图像分析，并在100小时自主探索中生成52份研究报告，其中22个机制假设被优先考虑。该方法显著扩展了实验室自动化，提供可扩展、可审计的自主路径。"
source: biorxiv
selection_source: fresh_fetch
motivation: 现有实验室计算工作流依赖手动操作专业软件和视觉检查，效率低下且知识整合困难。
method: Orion融合大语言模型与终端控制、GUI操作及多步推理，共享计算环境中自动执行数据分析和工具调用。
result: "基准任务准确率超90%；掌握CellProfiler和QuPath分析；100小时自主探索生成52份报告、22个可信假设。"
conclusion: 使用计算机的AI代理能大幅扩展实验室自动化，实现从成像数据到分析报告和生物学假设的自主路径。
---

## 摘要
实验室发现越来越依赖于将实验数据与分析、解释和后续假设连接起来的计算工作流。然而，这些工作流仍受限于劳动密集型的专业软件使用、通过图形用户界面进行视觉检查以及跨多个来源的知识整合。在这里，我们介绍了Orion，一个用于生物医学图像分析和解释的计算机使用型AI代理，通过自动化实验室工作的计算层，迈向实验室自动化。Orion将大型语言模型与终端执行、GUI控制和自适应多步推理结合在一个共享的计算环境中。它可以检查视觉数据、操作标准科学软件、挖掘网络资源并进行端到端的分析和解释工作流，而无需定制软件集成。在基准测试中，Orion在生物医学数据库和文献检索任务上达到了超过90%的准确率，学习了分别使用流行工具CellProfiler和QuPath进行细胞和组织图像的定量分析，并促进了实验成像数据中的自主发现。在100小时的大规模扰动成像数据集自主探索中，Orion生成了52份研究报告，其中人类科学家审查后优先选择了22个合理的机制假设。这些结果表明，使用计算机的AI代理可以显著扩展实验室自动化的范围，为从实验成像数据到定量分析、报告和生物学基础的假设提供了一条可扩展且可审计的路径。
