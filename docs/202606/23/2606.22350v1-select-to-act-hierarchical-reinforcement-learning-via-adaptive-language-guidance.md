...
tags: ["query:rl"]
score: 7.0
evidence: 基于语言指令的分层强化学习用于序列决策
tldr: 强化学习面临样本效率低的问题，现有方法将自然语言指令作为单一条件输入，忽视了指令在不同决策阶段的相关性。本文提出HRLLI层次强化学习框架，将指令分解为多个片段，高层策略根据当前状态动态选择最相关的指令片段，低层策略在所选片段指导下执行动作。在RTFM基准上，HRLLI显著优于多种指令条件强化学习基线，表明显式建模自适应指令选择能有效提升强化学习效果。
source: arxiv
...
motivation: 现有方法将完整指令作为单一条件输入，忽略了指令在不同决策阶段的相关性，导致样本效率低下。
method: 提出Select-to-Act范式，高层策略动态选择当前最相关的指令片段，低层策略基于该片段执行环境动作，两层次联合学习。
result: 在RTFM基准上，HRLLI超越了所有指令条件强化学习基线，验证了自适应指令选择的有效性。
conclusion: 显式建模指令的阶段选择性能够提升复杂环境中强化学习的决策质量和样本效率。
---

## 摘要
强化学习（RL）已被广泛应用于序列决策，但由于与环境的交互成本高，样本效率往往较低。近期少量工作开始探索通过利用自然语言指令中的外部知识来提高RL效率。然而，现有的少数方法通常将整个指令作为单一条件输入，未能考虑语言指导的阶段依赖性，尤其是在复杂环境中。本文提出分层强化学习与语言指令（HRLLI），一种明确将自然语言指令建模为决策过程中动态可选择的语义指导的分层RL框架。HRLLI将指令分解为一组分段指导元素，每个指令片段可能在与环境交互的不同阶段变得相关。然后，在“选择-行动”范式下构建了一种新颖的分层RL策略结构：高层语义策略作为指导选择器，选择与当前状态最相关的指令片段来指导低层智能体的决策，而低层策略则基于所选指导执行环境动作。两层策略同时学习，以最大化与环境交互的增强预期回报。这种设计使智能体能够在交互过程中自适应地将语言指令具体化为阶段特定决策。在指令密集型RTFM基准上的实验表明，HRLLI持续优于强指令条件RL基线，证明显式建模自适应指令选择显著提高了RL的有效性。
