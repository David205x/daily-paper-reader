...
tldr: "智能搜索中基于结果强化学习的监督稀疏，而传统知识蒸馏因分布差异和风格干扰效果不佳。本文提出多智能体协议蒸馏（MAPD），通过离线多智能体系统将搜索过程转换为结构化JSON协议作为中间表示，联合蒸馏信号与稀疏RL目标训练学生模型。在7个QA基准上，Qwen3-1.7B和Qwen3-4B平均成功率达39.4%和44.4%，优于竞争方法。MAPD能鲁棒泛化到不同专有教师，同时缓解风格漂移与冗长退化。"
source: arxiv
...
motivation: 现有智能搜索中基于结果的强化学习监督稀疏，且直接蒸馏专有模型因分布差异和风格干扰难以迁移核心推理能力。
method: 提出多智能体协议蒸馏（MAPD），通过离线MAS将搜索轨迹转为结构化JSON协议作为中间表示，联合密集蒸馏信号与稀疏RL目标训练学生。
result: "在7个QA基准上，MAPD使Qwen3-1.7B和Qwen3-4B平均成功率达39.4%和44.4%，优于基线，且泛化到多种专有教师。"
conclusion: MAPD弥合了专有与开源模型在智能搜索中的分布差距，提升学生性能的同时避免风格退化。
---

## 摘要
智能搜索通过将多步推理与检索交织使大语言模型能够解决知识密集型任务，但使用基于结果的强化学习（RL）进行优化仅提供稀疏监督。知识蒸馏可提供更密集的指导，而具有强大推理能力的高级专有模型是很有前途的教师。虽然从专有模型蒸馏可以增密这种监督信号，但传统的logit匹配因隐藏的logit和分词器不匹配而无法实现，而原始自然语言轨迹模仿则传递表面风格伪影而非核心推理能力。为解决异构蒸馏问题并弥合分布差距，我们提出多智能体协议蒸馏（MAPD），这是一种联合蒸馏和RL框架，使用结构化的、风格归一化的协议作为中间表示。离线多智能体系统（MAS）分解每个查询，检索支持证据，修复失败的搜索，并将生成的探索轨迹转换为包含任务类型、推理计划和提取性基础事实的JSON协议。训练期间，该协议仅提供给学生策略的特权分支，其令牌分布提供密集的蒸馏信号以及稀疏的RL目标。在七个QA基准上的广泛评估表明，MAPD持续优于竞争性蒸馏和RL，在Qwen3-1.7B上平均成功率达到39.4%，在Qwen3-4B上达到44.4%。至关重要的是，该框架在不同专有教师之间具有强泛化能力，同时有效缓解学生策略的风格漂移和冗长退化。
