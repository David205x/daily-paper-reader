...
title_zh: CHILL-Harness：面向长周期智能体的反事实控制学习以实现高效推理
...
tldr: 长时程智能体框架常采用手工或全局固定策略，导致计算浪费和任务成功率下降。CHILL-Harness将自适应编排建模为因果学习问题，通过置信加权证据估计干预优势，仅执行充分有利的调整。在信息检索、软件工程等任务中，该方法在保持或提升成功率的同时，大幅降低token消耗和执行时间。
source: arxiv
...
motivation: 现有框架依赖固定策略，无法适应任务和执行状态变化，造成不必要的计算开销和性能损失。
method: 提出因果干预效应学习估计工作流优势，通过优势实现因果编排自适应分配反事实推理，结合成功保持目标和优势边界约束。
result: 在异构长时程任务中，成功率不变或提升，token消耗和执行时间显著减少。
conclusion: CHILL-Harness通过因果干预学习实现高效自适应框架编排，平衡效率与可靠性。
---

## 摘要
智能体控制框架已成为现代大语言模型智能体的运行基础设施，协调上下文、工具、验证和执行控制，将潜在模型能力转化为可靠的长周期行为。然而，可靠的长周期行为要求控制框架适应任务需求、执行环境和不断变化的执行状态，而当前控制框架主要依赖手工设计或全局固定的策略；这种不匹配表现为不必要的计算开销，在不利情况下还会降低任务成功率。为解决这一局限，我们将控制系统中实现自适应编排的任务形式化为因果学习问题，并提出了面向长周期智能体的反事实控制干预学习（CHILL-Harness）。CHILL-Harness在编排层实施干预，实现优势引导的工作流自适应，从而在保持任务性能的同时提升推理和执行效率。具体而言，我们开发了因果干预效应学习作为CHILL-Harness的效应估计组件，从置信度加权的执行证据中估计干预相关的工作流优势，并识别有利的工作流调整。进一步，我们引入了优势实现因果编排作为其实现组件，自适应分配反事实推理，仅实现有足够预期优势支持的工作流调整。最后，我们在CHILL-Harness中融入了成功保持目标和优势边际授权约束，以促进可靠的自适应。在涵盖信息检索、软件工程和终端交互的异构长周期任务上的大量实验表明，CHILL-Harness在显著减少令牌消耗和执行时间的同时，始终能保持或提升任务成功率。
